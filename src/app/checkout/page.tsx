...
export default async function CheckoutPage() {
  ...
  return (
    <main className="mx-auto w-full max-w-5xl px-5 py-10 md:py-16 2xl:max-w-7xl">
      ...
      {open ? <CheckoutSteps /> : <PaymentsClosed />}
    </main>
  );
}

function PaymentsClosed() {
  return (
    <div className="mt-8 max-w-prose">
      ...
      <p className="mt-4">
        We&apos;re no longer accepting online payments or new participants for
        the Acquaintance Party.
      </p>
      <p className="mt-3">
        Already paid? Your ticket still works — use{" "}
        <Link href="/find" className="font-semibold underline">
          Find your ticket
        </Link>{" "}
        with your student ID and the email you registered with. Something wrong
        with it? {EVENT.contact}
      </p>
    </div>
  );
}
