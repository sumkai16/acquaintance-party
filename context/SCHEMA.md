...
| amount | integer | NOT NULL, `>= 0` | **Centavos**, never a float — what this ticket costs its holder: `priceFor(ticket_rate)` (`src/lib/registrations/rates.ts`), i.e. `EVENT.ticketPriceCentavos` for a regular ticket. `>= 0` since `0022` — a free ticket is a real ₱0 row |
| ticket_rate | text | NOT NULL, default `'regular'`, `'regular'` \| `'officer'` \| `'free'` | Added in `0022_ticket_rates.sql`. `officer` is ₱250 (`EVENT.officerPriceCentavos`), `free` is ₱0. Both are recorded only by an **admin**, only on the single `/admin/walk-in` form (the action checks the role itself — hiding the picker isn't the gate), always paid in full so approved on the spot with a QR, never partial. Bulk import and staff quick-entry stay regular. Constraint `ticket_rate_matches_amount`: `(ticket_rate = 'free') = (amount = 0)`. No new cash logic was needed — every cash total already sums `amount_paid`, so an officer ticket counts ₱250 and a free one ₱0. A free ticket issues no receipt (the `receipts` table's own `amount > 0` check would reject ₱0) but still gets the QR email |
